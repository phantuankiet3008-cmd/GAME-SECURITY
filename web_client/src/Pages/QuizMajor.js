...
function QuizMajor() {
  const questions = questionChild.Questions_Adult;
  ...
  return (
    ...
    <div>
      <div className="">
        <img src={require("../Assets/logo.png")} className='quiz-logo' alt="Logo"/>
      </div>
      <Button onClick={() => setShowLanguageModal(true)} className='language-button button-user'>
        {labels.language}: {language.toUpperCase()}
      </Button>
      <Modal show={showLanguageModal} onHide={() => setShowLanguageModal(false)} centered className='language-modal'>
        <Modal.Header closeButton>
          <Modal.Title>{labels.chooseLanguage}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <ButtonGroup className='language-options'>
            <Button className='button-user' active={language === 'en'} onClick={() => changeLanguage('en')}>English</Button>
            <Button className='button-user' active={language === 'vi'} onClick={() => changeLanguage('vi')}>Tiếng Việt</Button>
          </ButtonGroup>
        </Modal.Body>
        <Modal.Footer>
          <Button className='button-user' onClick={() => setShowLanguageModal(false)}>{labels.close}</Button>
        </Modal.Footer>
      </Modal>
      <div id='ready'>
        <div className='' style={{marginTop: '5%'}}>
              <h1 className='quiz-heading text-light'>
                {language === 'vi'
                  ? `${labels.welcome} ${username} đến với quiz ${labels.advancedTitle} !`
                  : `${labels.welcome} ${username} to the ${labels.advancedTitle} quiz !`}
              </h1><br/>
              <h2 className='text-center text-light'>{labels.ready}</h2><br/><br/>
              <Button onClick={showQuizz} style={{backgroundColor: '#01d976', borderColor: '#01d976'}} className='button-user rounded-pill col-md-2 start-button'>{labels.start}</Button><br/><br/>
        </div><br/>
        <div className='offset-md-3 col-size how-to-play'>
          <div className='d-flex how-to-title'>
            ...
            <img src={require("../Assets/Bot.png")} className='bot-icon' alt="Logo"/>
          </div>
          <p className='text-center how-to-copy text-light'>{labels.instructions}<br/>{labels.haveFun}</p>
        </div>
      </div>
      ...
      <div className='col-md-1 offset-md-4' id="looseIcon" style={{marginTop: '2%', display: 'none'}}>
        ...
        <div className='score-display text-light'>
            <h1>{username} {labels.score} : {score}</h1>
        </div>
        </div>

      <div className='cardContainer'>
        {translatedQuestions.map((character, index) => (
          <div key={index} id={'card'+index} style={{display: (index === currentIndex) ? 'grid' : 'none'}}>
      ...
      <button className='button-go' style={{ backgroundColor: !canSwipe && '#c3c4d3' }} onClick={() => swipe('left')}>{labels.answer} 1</button>
      <button className='button-aqua' style={{ backgroundColor: !canSwipe && '#c3c4d3' }} onClick={() => swipe('right')}>{labels.answer} 2</button>
      <button className='button-fu' style={{ backgroundColor: !canSwipe && '#c3c4d3' }} onClick={() => swipe('up')}>{labels.answer} 3</button>
      <button className='button-lightsalmon' style={{ backgroundColor: !canSwipe && '#c3c4d3' }} onClick={() => swipe('down')}>{labels.answer} 4</button>
      </div>
      ...
      <div id="spin" style={{display: 'none', marginTop: '10%'}}>
        ...
        <h5 style={{color:"white", fontWeight: 'bold'}}>{labels.loadingResults}</h5>
      </div>
      <div id="details" style={{marginTop: '2%', display: 'none'}}>
      <Button href='/score-major' style={{backgroundColor: '#01d976', borderColor: '#01d976', fontWeight: 'bold'}} className='rounded-pill col-md-2 button-user leaderboard-button'>{labels.viewLeaderboard}</Button><br/><br/>
        <div className='results-table-wrap'>
                <Table bordered className='dark-score-table' style={{border: '2px solid #01d976', color: 'white', boxShadow: '0px 0px 20px 0px rgba(1,217,118, 0.8)'}}>
                <thead style={{border: '2px solid #01d976'}}>
                  <tr style={{border: '2px solid #01d976'}}>
                    <th style={{border: '2px solid #01d976'}} width='20%'>{labels.question}</th>
                    <th style={{border: '2px solid #01d976'}} width='10%'>{labels.goodAnswer}</th>
                    <th style={{border: '2px solid #01d976'}} width='20%'>{labels.details}</th>
                    <th style={{border: '2px solid #01d976'}} width='4%'>{labels.yourAnswer}</th>
                  </tr>
                </thead>
